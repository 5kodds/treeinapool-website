...
export const metadata: Metadata = {
  title: "Process",
  description: "How a TreeInAPool engagement runs, stage by stage, plus answers to the top pre-sales questions.",
};
      ...
      <section className="pt-14 pb-10 md:pt-20 md:pb-12">
        <Container>
          ...
          <p className="mt-7 max-w-[56ch] text-[17px] leading-7 text-muted">
            Four stages, each with a fixed duration and a clear answer to &quot;what do I get, and
            ...
            {PROCESS_STAGES.map((stage) => (
              <Frame key={stage.n} className="p-6">
                ...
                <div className="mt-3 flex items-baseline justify-between gap-3">
                  <h2 className="text-2xl uppercase leading-[26px] tracking-wide">{stage.name}</h2>
                  <span className="whitespace-nowrap text-sm text-muted">{stage.duration}</span>
                </div>
                <p className="mt-3 text-[15px] leading-6 text-muted">{stage.body}</p>
                <p className="mt-4 border-t border-[var(--color-divider)] pt-4 text-sm leading-6">
                  <span className="font-semibold text-[var(--color-accent-700)]">You do: </span>
                  {stage.client}
                </p>
              </Frame>
            ))}
        ...
        <Container className="grid gap-10 md:grid-cols-[7fr_5fr] md:items-center">
          <div>
            ...
            <p className="mt-4 max-w-[52ch] text-base leading-7 text-muted">
              Thirty minutes, and you leave with a written scope, a price, and a date.
            </p>
          </div>
