...
| D5 | Outcome numbers, and permission to name Baniri Technologies | `content/case-studies/*.md` | The invented studies are gone. Farm Buddy (own product), Vuvu.ng (for Baniri Technologies) and AfroMadeIt Global (own company, founder is Co-Founder & COO) replace them, each labelled for what it actually is. **No metrics anywhere**, so the outcome band stays hidden until real figures are confirmed. One gap marked `[ Confirm ]`: the Vuvu duration. **Naming a client publicly needs their agreement**, so confirm Baniri are happy to be named before this goes in front of anyone |
