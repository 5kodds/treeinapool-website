...
`FORMSPREE_ENDPOINT` is unset, so contact and rebuild enquiries are
accepted, validated, and **not delivered anywhere** — the visitor is shown
a pre-filled mailto fallback so a real lead still reaches
`hello@treeinapool.com`. Set the variable (or swap the branch in
`src/app/api/contact/route.ts` for another provider) before driving traffic
to the site. See the README.
